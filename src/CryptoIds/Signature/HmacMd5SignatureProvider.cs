...
{
    private const int ConstSignatureLengthInBytes = 16;
    public static int SignatureLengthInBytes => ConstSignatureLengthInBytes;

    public static readonly HmacMd5SignatureProvider Instance = new();
    ...
    public int Sign<T>(T id, ReadOnlySpan<byte> key, Span<byte> destination) where T : unmanaged
    {
        ...
        _ = HMACMD5.TryHashData(key, buffer, destination, out int retVal);
        return retVal;
    }
